...
We've set you up with an [SQLite database](https://keystonejs.com/docs/apis/config#sqlite) for ease-of-use. If you want to use PostgreSQL, you can!
...
For more on database configuration, check out our [DB API Docs](https://keystonejs.com/docs/apis/config#db).
...
To create a custom API, install the following dependencies:

```bash
npm install axios @graphql-tools/schema
```

### Allow Admin Access in `keystone.ts`

To restrict the Admin UI to only admin users, add the following to your `keystone.ts` file:
